...
class Matcher:
    ...
    ds : str
        Name of variable that is the dataset indicator
    exclude : list  (optional)
        List of variables to ignore in regression/matching.
        Useful for unique idenifiers
    """

    def __init__(self, test, control, yvar, ds, formula = None, exclude = []):
        ...
        self.data = t.dropna(axis = 1, how = 'all').append(c.dropna(axis = 1, how = 'all'))
        self.control_color = "#1F77B4"
        self.test_color = "#FF7F0E"
        
        self.yvar = yvar
        self.ds = ds
        self.exclude = exclude + [self.yvar, self.ds] + aux_match
        ...
        print('n base: ', self.testn)
        print('n to match: ', self.controln)
        
        
        
    def balanced_sample(self, data = None):
        """
        Randomly selects an equal amount of subjects from each data set
        """
        if not data:
            data = self.data
        minor, major = self.data[self.data[self.ds] == self.minority], \
                       self.data[self.data[self.ds] == self.majority]
    ...
    def fit_scores(self, balance = True, nmodels = None):
        ...
        if balance:
            if nmodels is None:
                ...
                nmodels = int(np.ceil((len(self.control) / len(self.test)) / 10) * 10)
            self.nmodels = nmodels
            i = 0
            errors = 0
            
            # loop through all the models to get average accuracy, use the GLM
    ...
    def plot_scores(self):
        ...
        # plotting propensity scores per data set
        sns.distplot(self.data[self.data[self.ds] == 2].scores, label = 'Control')
        sns.distplot(self.data[self.data[self.ds] == 1].scores, label = 'Test')
    ...
    def match(self, threshold = 0.001, nmatches = 1, method = 'min', max_rand = 10):
        ...
        test_scores = self.data[self.data[self.ds] == 1][['scores']]
        ctrl_scores = self.data[self.data[self.ds] == 2][['scores']]
        result, match_ids = [], []
    ...
    def prop_retained(self):
        """
        Returns the proportion of test data retained after matching
        """
        return len(self.matched_data[self.matched_data[self.ds] == self.minority]) * 1.0 / \
               len(self.data[self.data[self.ds] == self.minority])
    ...
    def prep_prop_test(self, data, var):
        ...
        """
        counts = data.groupby([var, self.ds]).count().reset_index()
        table = []
        for t in (1, 2):
            os_counts = counts[counts[self.ds] == t]\
                                     .sort_values(var)
            cdict = {}
    ...
    def prop_test(self, col):
        ...
        if col not in self.exclude:
            ...
            print("{} is an excluded variable".format(col))
            
            
    
    def compare_cat(self, cols, return_table = False):
        """
        ...
        def prep_plot(data, var, colname):
            t, c = data[data[self.ds] == 1], data[data[self.ds] == 2]
    ...
    def compare_cont(self, cols, save = False, return_table = False):
        """
        ...
        for col in cols:
            ...
            tra = self.matched_data[self.matched_data[self.ds] == 1][col]
            coa = self.matched_data[self.matched_data[self.ds] == 2][col]
